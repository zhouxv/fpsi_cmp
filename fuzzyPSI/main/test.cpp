...
// ───────────────────────────────────────
void runStandaloneReceiver(const char *addr, u64 cmpsize, u64 dim, u64 delta,
                           u64 metric, u64 Cmp_len, u64 numThreads) {
  ...
  macoro::sync_wait(receiver.setUp(cmpsize, dim, delta, metric, Cmp_len, prng,
                                   chl, numThreads));
  ...
  DEBUG_LOG("Computation time: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(recvEnd -
                                                                     recvBegin)
                   .count()
            << " ms");
  DEBUG_LOG("Communication: "
            << chl.bytesSent() + chl.bytesReceived() - setup_comm << " bytes");
  // chl.close();
}
...
// ───────────────────────────────────────
void runStandaloneSender(const char *addr, u64 cmpsize, u64 dim, u64 delta,
                         u64 metric, u64 Cmp_len, u64 numThreads) {
  ...
  // sender.setUp(cmpsize, dim, delta, metric, Cmp_len, hashes, prng,
  // numThreads);
  macoro::sync_wait(sender.setUp(cmpsize, dim, delta, metric, Cmp_len, prng,
                                 chl, numThreads));
  ...
  //         throw std::runtime_error("mIMT Sender::e and d mismatch at index
  //         "+std::to_string(i));
  //     }
  // }
  ...
  DEBUG_LOG("Sender done.");
  DEBUG_LOG("Computation time: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(sendEnd -
                                                                     sendBegin)
                   .count()
            << " ms");
  DEBUG_LOG("Communication: "
...
void printUsage(const char *prog) {
  std::cout << "Usage: " << prog << " [mode] [options]\n"
            ...
            << "Options:\n"
            << "  --addr IP:PORT          (default: 127.0.0.1:1212)\n"
...
int main(int argc, char **argv) {
  ...
  std::string addr = "127.0.0.1:1212";
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--mode" && i + 1 < argc)
      ...
      cmpsize = std::stoull(argv[++i]);
    else if (arg == "--dim" && i + 1 < argc)
      dim = std::stoull(argv[++i]);
    else if (arg == "--delta" && i + 1 < argc)
      delta = std::stoull(argv[++i]);
    else if (arg == "--metric" && i + 1 < argc)
      metric = std::stoull(argv[++i]);
    else if (arg == "--cmp-len" && i + 1 < argc)
      Cmp_len = std::stoull(argv[++i]);
    else if (arg == "--threads" && i + 1 < argc)
      numThreads = std::stoull(argv[++i]);
  ...
  try {
    if (mode == "local") {
      ...
      runStandaloneSender(addr.c_str(), cmpsize, dim, delta, metric, Cmp_len,
                          numThreads);
    } else if (mode == "receiver") {
      runStandaloneReceiver(addr.c_str(), cmpsize, dim, delta, metric, Cmp_len,
                            numThreads);
    } else {
