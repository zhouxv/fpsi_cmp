...
namespace CmpFuzzyPSI
{
    Proto FuzzyPsiSender::runL1(span<block> inputs, Socket& chl){
        ...
        u64 offlineComm = 0;
        u64 onlineComm = 0;
        u64 modL = ((mDelta * mDim + 1)%2 == 0) ? (mDelta * mDim + 2) : (mDelta * mDim + 1);
        ...
        //offline ROT
        std::vector<std::vector<u64>> rot_prime(TableSize * mDim, std::vector<u64>(2, 0));
        ...
        std::vector<u64> cmp_inputs_bin(TableSize*mDim*3);//y-delta, y ,y+delta for L1
            ...
            if (!cuckoo.mBins[i].isEmpty()) {
                ...
                for (u64 j=0; j < mDim; j++){
                    ...
                    if (y_hat - y_org + mDelta > m_min_1)
                        cmp_inputs_bin[3*i*mDim + 3*j + 2] = (m_min_1 + y_org) & m_min_1;
                    else 
        ...
        Begin = timer.setTimePoint("FuzzyPsiSender::sencond OPPRF and OT exchange begin");
        ...
        std::vector<u8> Opprf_val_data_prime(mSenderSize * valsize_prime_byte);// length d vector in each posotion, length of vector is peqtLength-d + Cmp_len*d
        MatrixView<u8> Opprf_val_prime(Opprf_val_data.data(), mSenderSize, valsize_prime_byte);
        ...
        macoro::sync_wait(mOpprfReceiver2.receive(mRecverSize*3, CuckooKeys, Opprf_val_prime, mPrng, mNumThreads, chl));
        ...
        for (u64 i=0; i<TableSize; i++){
            ...
            if (!cuckoo.mBins[i].isEmpty()) {
                ...
                for (u64 j=0; j < mDim; j++){
                    ...
                    //     std::cout << m_hat_r[i*mDim+j];
                    //     std::cout << std::endl;
                    // }

                    u64 m_i_s = mod.sub(((u64*)&inputs[b*mDim + j])[0], m_hat_r[i*mDim+j]);
                    ...
                    if (t_i_prime[i*mDim+j]){
                        distance_share[2*i+1] = mod.sub(distance_share[2*i+1], rot_prime[i*mDim+j][0]);
                        // distance_share[2*i+1] = mod.add(distance_share[2*i+1], rot_prime[i*mDim+j][0]);
                        m_hat_s[i*mDim+j] = mod.sub(m_hat_s[i*mDim+j], rot0prime_sub_rot1prime[i*mDim+j]);
                    } else {
                        distance_share[2*i+1] = mod.sub(distance_share[2*i+1], rot_prime[i*mDim+j][1]);
                        // distance_share[2*i+1] = mod.add(distance_share[2*i+1], rot_prime[i*mDim+j][1]);
                        m_hat_s[i*mDim+j] = mod.add(m_hat_s[i*mDim+j], rot0prime_sub_rot1prime[i*mDim+j]);
                    }    
                    ...
                    for (u64 k=0; k< modLLength; k++){
                        ...
                        m_hat_s_message[i*valsize_prime_byte + byteIndex] ^= (u64)bit << bitIndex;
                    }
                }
            } else {
                distance_share[2*i+1]=mPrng.get();
            }
            distance_share[2*i+1] = mod.sub(distance_share[2*i+1], rot_sum_d[i]);
        ...
        for (u64 i=0; i<TableSize; i++){
            for (u64 j=0; j<peqtLength; j++){
                if (j < (peqtLength - mDim -1)){
                    ...
                    peqtSend[i*peqtLength + j] = output[2*(i*mDim + (j - (peqtLength - mDim - 1)))];
                } else {
        ...
        //             //     std::cout << "bin" << i << " item " << b <<" dim " << j << ": " << "share: " << output[2*(i*mDim+j)+1] << " " << t_i_prime[i*mDim+j] ;
        //             //     std::cout << std::endl;
        //             //     std::cout << " part 1: " << tmp << " " << mod.add(tmp, m_i_s) << " " << mod.sub(tmp, m_i_s);
        //             //     std::cout << std::endl;
        //             //     std::cout << " part 2: " << tmp0 ;
        //             //     std::cout << std::endl;
        //             //     std::cout << " m_i_s: " << m_i_s << " m_hat_s: " << m_hat_s[i*mDim+j];
        //             //     std::cout << std::endl;
        //             // }
                    
        //             // std::cout << mod.sub(((u64*)&inputs[b*mDim])[0], m_hat_r[i*mDim]);
        //             std::cout << std::endl;
        //         }
        //     }
        // }
    ...
    Proto FuzzyPsiReceiver::runL1(span<block> inputs, Socket& chl){
        ...
        u64 valsize_prime = modLLength * mDim;
        u64 valsize_prime_byte = (valsize_prime + 7) / 8;
        std::vector<u8> Opprf_val_data_prime(Identifiers.size()*valsize_prime_byte*3, 0);// length d vector in each posotion, length of vector is peqtLength-d + Cmp_len*d
        MatrixView<u8> Opprf_val_prime(Opprf_val_data_prime.data(), Identifiers.size()*3, valsize_prime_byte);
        ...
        for (u64 i = 0; i < TableSize; ++i){
            ...
            for (u64 p = 0; p < size; ++p){
                ...
                for (u64 j=0; j < mDim; j++){
                    ...
                    u64 tmp = mod.sub(((u64*)&inputs[vidx*mDim + j])[0], m_i_r[i*mDim+j]);
        ...
        macoro::sync_wait(mOpprfSender2.send(mSenderSize, Opprf_key, Opprf_val_prime, mPrng, mNumThreads, chl));
        ...
        for (u64 i = 0; i < TableSize; ++i){
            for (u64 j=0; j < mDim; j++){
                for (u64 k=0; k< modLLength; k++){
                    u64 byteIndex = (j*modLLength + k) / 8;
                    u64 bitIndex = (j*modLLength + k) % 8;
                    bool bit = (m_hat_s_message[i*valsize_prime_byte + byteIndex] >> (bitIndex)) & 1;
                    m_hat_s[i*mDim + j] ^= (bit << k);
                }
                distance_share[i] = mod.sub(distance_share[i], rot0_add_rot1[i*mDim + j]);
        ...
        //             //     std::cout << "bin" << i << " item " << b <<" dim " << j << ": " << "shares:" << output[2*(i*mDim+j)+1]<< " " << s_prime[i*mDim+j] ;
        //             //     std::cout << std::endl;
        //             //     // std::cout << mod.sub(tmp, rot[i*mDim + j][0]) << " " << mod.sub(tmp, rot[i*mDim + j][1]);
        //             //     std::cout << " part 2 " << tmp ;
        //             //     std::cout << std::endl;
        //             //     std::cout << " part 1 " << tmp0 << " " <<  mod.sub(tmp0, m_i_r[i*mDim + j]) << " " << mod.add(tmp0, m_i_r[i*mDim + j]);
        //             //     std::cout << std::endl;
        //             //     std::cout << " m_i_r: " << m_i_r[i*mDim + j] << " m_hat_s: " << m_hat_s[i*mDim + j];
        //             //     std::cout << std::endl;
        //             // }
