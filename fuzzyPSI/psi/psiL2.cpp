  ...
  for (u64 i = 0; i < TableSize; i++) {
    ...
    if (!cuckoo.mBins[i].isEmpty()) {
      ...
      for (u64 j = 0; j < mDim; j++) {
        ...
        b_prime[i * mDim + j] =
            mod.sub(a_prime, ((u64 *)&inputs[dataindex * mDim + j])[0]);
        u64 tmp = mod.sub(b_prime[i * mDim + j] * b_prime[i * mDim + j],
                          2 * ole_d[i * mDim + j]);
        distance_share[2 * i + 1] = mod.add(distance_share[2 * i + 1], tmp);
        b_prime[i * mDim + j] =
            mod.sub(b_prime[i * mDim + j], ole_b[i * mDim + j]);
      }
    ...
    for (u64 j = 0; j < peqtLength; j++) {
      if (j < (peqtLength - mDim - 1)) {
        ...
        peqtSend[i * peqtLength + j] =
            output[i * mDim + (j - (peqtLength - mDim - 1))];
      } else {
...
Proto FuzzyPsiReceiver::runL2(span<block> inputs, Socket &chl) {
  ...
  for (u64 i = 0; i < TableSize; ++i) {
    ...
    for (u64 p = 0; p < size; ++p) {
      ...
      for (u64 j = 0; j < mDim; j++) {
        u64 tmp =
            mod.add(((u64 *)&inputs[dataindex * mDim + j])[0], ole_a[i * mDim + j]);
        for (u64 k = 0; k < modLLength; k++) {
          u64 byteIndex = (j * modLLength + k + preidx) / 8;
          u64 bitIndex = (j * modLLength + k + preidx) % 8;
          bool bit = (tmp >> k) & 1;
  ...
  for (u64 i = 0; i < TableSize; ++i) {
    for (u64 j = 0; j < mDim; j++) {
      distance_share[i] =
          mod.sub(distance_share[i], ole_a[i * mDim + j] * ole_a[i * mDim + j]);
      distance_share[i] = mod.sub(distance_share[i], 2 * ole_c[i * mDim + j]);
      distance_share[i] = mod.add(distance_share[i], 2 * ole_a[i * mDim + j] *
                                                         b_prime[i * mDim + j]);
    }
    for (u64 k = 0; k < modLLength; k++) {
