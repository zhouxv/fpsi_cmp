...
namespace CmpFuzzyPSI
{
    ...
    void details::FuzzyPsiBase::init(u64 senderSize, u64 recverSize, u64 statSecParam, u64 dim, u64 metric, u64 delta, u64 LorH, block seed, u64 numThreads, bool useReducedRounds)
    {
    ...
    Proto FuzzyPsiSender::runLinfty(span<block> inputs, Socket& chl){
        ...
        std::vector<u8> Opprf_val_data(CuckooKeys.size()*valsize_byte);// length d vector in each posotion, length of vector is peqtLength-d + Cmp_len*d
        MatrixView<u8> Opprf_val(Opprf_val_data.data(), CuckooKeys.size(), valsize_byte);
        auto v_s = BitVector(mmIMTSender.mShareSize); // copy the opprf value from TableSize*d*cmp_len to 2*TableSize*d*cmp_len
        auto masks = BitVector(TableSize*(peqtLength-mDim));
        std::vector<u64> cmp_inputs_bin(TableSize*mDim*2);//[y-delta,y+delta] for L_infty
        ...
        // " outputsize: " << output.size() <<  " moutputsize: " << mmIMTSender.mOutputSize);
        ...
        for (u64 i=0; i<TableSize; i++){
            for (u64 j=0; j<peqtLength; j++){
                if (j < (peqtLength - mDim)){
                    ...
                    peqtSend[i*peqtLength + j] = output[i*mDim + (j - (peqtLength - mDim))];
                }
            }
        }
        ...
        //         u64 b = cuckoo.mBins[i].idx();
        //         if (b < 5){
        //             std::cout << "bin" << i << " item " << b << ": " << peqtoutput[i] << " " ;
        //             for (u64 j=0; j<peqtLength; j++){
        //                 std::cout <<  peqtSend[i*peqtLength + j];
        //             }
        //             std::cout << std::endl;

        //             // std::cout << " bin " << i << " cmpinput " << b << ": " << cmp_inputs_bin[2*i*mDim] << " "
    ...
    Proto FuzzyPsiReceiver::runLinfty(span<block> inputs, Socket& chl){
        ...
        DEBUG_LOG("mIMT done.");
        // DEBUG_LOG("TableSize:" << TableSize << " masksize: " << masks.size() << 
        // " outputsize: " << output.size() <<  " moutputsize: " << mmIMTReceiver.mOutputSize);
        ...
        //             // std::cout << " bin " << i << " cmpinput " << b << ": " << (((u64*)&inputs[b*mDim])[0] & m_min_1) << std::endl;
        //         }
        //     }
        // }
